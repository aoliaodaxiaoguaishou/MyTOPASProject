...
7.2. Run the following commands: 
        ...
        cd geant4-build
        cmake .. -DGEANT4_INSTALL_DATA=OFF -DGEANT4_BUILD_MULTITHREADED=ON -DCMAKE_INSTALL_PREFIX=../geant4-install -DCMAKE_PREFIX_PATH=/usr/lib/qt5 -DGEANT4_USE_QT=ON -DGEANT4_USE_OPENGL_X11=ON -DGEANT4_USE_RAYTRACER_X11=ON 
...
8.3. Build GDCM with the following commands:
        ...
        cd gdcm-build
        cmake .. -DGDCM_BUILD_SHARED_LIBS=ON -DGDCM_BUILD_DOCBOOK_MANPAGES:BOOL=OFF  -DCMAKE_INSTALL_PREFIX=../gdcm-install 
        sudo make -j20 install

8.4. With GDCM built and installed, you can continue with the OpenTOPAS installation:

        cd $HOME/Applications/TOPAS
        sudo rm -rf OpenTOPAS-install OpenTOPAS-build
        mkdir OpenTOPAS-{build,install}
        cd OpenTOPAS-build
        export Geant4_DIR=/home/liangkun/geant4/geant4-v11.1.3/geant4-install   # 需要改成你的geant4-install
        export GDCM_DIR=/home/liangkun/GDCM/gdcm-2.6.8/gdcm-install # 改成你的gdcm-install
        cmake .. -DCMAKE_INSTALL_PREFIX=../OpenTOPAS-install -DTOPAS_EXTENSIONS_DIR="/home/liangkun/topas-nbio/TOPAS-nBio;/home/liangkun/topas-nbio/TOPAS-CellModels" -DCMAKE_BUILD_TYPE=Debug
 # 这一步我们可以用nbio的那个，加入扩展模块nbio+CellModel 																	
        sudo make -j27 install

## Step 9
Setup the environment. 

9.1.a This can be done manually as follows, however it needs to be repeated each time you open a new terminal window.
	# 你暂时在执行topas的时候，先执行这四行，记得改称你自己的目录，

        export QT_QPA_PLATFORM_PLUGIN_PATH=/home/liangkun/OpenTOPAS/OpenTOPAS-install/Frameworks
        export TOPAS_G4_DATA_DIR=/home/liangkun/G4Data
        export LD_LIBRARY_PATH=/home/liangkun/OpenTOPAS/OpenTOPAS-install/lib:$LD_LIBRARY_PATH
        export LD_LIBRARY_PATH=/home/liangkun/geant4/geant4-v11.1.3/geant4-install/lib:$LD_LIBRARY_PATH
...
10.a For those that decided to set the environment up manually, as described in Step 9.1.a, OpenTOPAS can now be run by accessing the executable located in `$HOME/Applications/TOPAS/OpenTOPAS-install/bin/topas`. For example, to run the simple OpenTOPAS example to test Qt visualization:
	用这里运行例子
        ...
        理论上在你的安装目录下  /openTopas-install下
