...
TsMyMedium::TsMyMedium(TsParameterManager* pM, TsExtensionManager* eM, TsMaterialManager* mM, TsGeometryManager* gM, TsVGeometryComponent* parentComponent, G4VPhysicalVolume* parentVolume, G4String& name) :
TsVGeometryComponent(pM, eM, mM, gM, parentComponent, parentVolume, name)
{
    // 获取外层椭球的三个半长轴
    fInnerRadius = fPm->GetDoubleParameter(GetFullParmName("InnerRadius"), "Length");
    fOuterRadius = fPm->GetDoubleParameter(GetFullParmName("OuterRadius"), "Length");
    fHalfHeight  = fPm->GetDoubleParameter(GetFullParmName("HalfHeight"), "Length");
...
G4VPhysicalVolume* TsMyMedium::Construct()
{
    BeginConstruction();

    // 创建外层的圆柱体 内半径，外半径，起始角度, 扫描角度(360表示完整的圆柱)
    G4Tubs* outerCylinder = new G4Tubs(fName, fInnerRadius, fOuterRadius, fHalfHeight, 0.*deg, 360.*deg);

    // 创建内层的立方体
    G4Box* innerBox = new G4Box(fName, fHLX, fHLY, fHLZ);

    // 创建椭球壳（外层椭球减去内层椭球）
    G4SubtractionSolid* myMedium = new G4SubtractionSolid("MyMedium", outerCylinder, innerBox, nullptr, G4ThreeVector(0, 0, -fHalfHeight+fHLZ));


    // 创建逻辑体
    fEnvelopeLog = CreateLogicalVolume(myMedium);
