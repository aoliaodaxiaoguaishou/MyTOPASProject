...
G4VPhysicalVolume* TsNucleus::Construct()
{
    ...
    fNucleusRadius = 0 *um; // 这里是我自己添加的参数
    if (fPm->ParameterExists(GetFullParmName("NucleusRadius")))
    fNucleusRadius = fPm->GetDoubleParameter(GetFullParmName("NucleusRadius"),"Length"); // 细胞半径
    ...
    fHilbertCurve3DRepeatX = fPm->GetIntegerParameter(GetFullParmName("HilbertCurve3DRepeatX")); // X方向体素数目
    fHilbertCurve3DRepeatY = fPm->GetIntegerParameter(GetFullParmName("HilbertCurve3DRepeatY")); // Y方向体素数目
    fHilbertCurve3DRepeatZ = fPm->GetIntegerParameter(GetFullParmName("HilbertCurve3DRepeatZ")); // Z方向体素数目
	...
	fVoxelAdd = 0 *nm;
	if (fPm->ParameterExists(GetFullParmName("VoxelAdd")))
	  fVoxelAdd = fPm->GetDoubleParameter(GetFullParmName("VoxelAdd"),"Length");
	...
	// 为了改成椭球形细胞核，新增加的参数
    fNucleusType = "Sphere"; // 细胞核类型
    if (fPm->ParameterExists(GetFullParmName("NucleusType")))
    fNucleusType = fPm->GetStringParameter(GetFullParmName("NucleusType"));
    if (!(fNucleusType=="Ellipsoid" || fNucleusType=="Sphere"))
    {
    G4cerr << "TOPAS is exiting due to a serious error in Geometry setup." << G4endl;
    G4cerr << "Unknown Nucleus Type: should be 'Sphere' or 'Ellipsoid'" << G4endl;
    }

    fNucleusRadiusX = 0.0 *um; //  细胞核细胞核X方向半长轴
    if (fPm->ParameterExists(GetFullParmName("NucleusRadiusX")))
    fNucleusRadiusX = fPm->GetDoubleParameter(GetFullParmName("NucleusRadiusX"),"Length");

    fNucleusRadiusY = 0.0 *um; //  细胞核细胞核Y方向半长轴
    if (fPm->ParameterExists(GetFullParmName("NucleusRadiusY")))
    fNucleusRadiusY = fPm->GetDoubleParameter(GetFullParmName("NucleusRadiusY"),"Length");

    fNucleusRadiusZ = 0.0 *um; //  细胞核细胞核Z方向半长轴
    if (fPm->ParameterExists(GetFullParmName("NucleusRadiusZ")))
    fNucleusRadiusZ = fPm->GetDoubleParameter(GetFullParmName("NucleusRadiusZ"),"Length");

    // Nucleus Orb 创建一个名为Nucleus名称的球体，半径为细胞半径
    // 这里创建了一个细胞核的几何体结构
    G4VSolid * Nucleus_solid = nullptr;
    if (fNucleusType == "Sphere")
    {
    Nucleus_solid = new G4Orb("Nucleus", fNucleusRadius);
    }
    else if (fNucleusType == "Ellipsoid")
    {
    Nucleus_solid = new G4Ellipsoid("Nucleus", fNucleusRadiusX, fNucleusRadiusY, fNucleusRadiusZ);
    }
    else
    {
    G4cerr << "TOPAS is exiting due to a serious error in Geometry setup." << G4endl;
	...
	G4double HilbertPointDistance = 40*nm+FiberEnvelopeLength + fVoxelAdd;
    ...
    G4double ParaContainerHalfSizeX = fVoxelLength*fHilbertCurve3DRepeatX/2; // 这里是要将Voxels容器X方向的半长
    G4double ParaContainerHalfSizeY = fVoxelLength*fHilbertCurve3DRepeatY/2; // 这里是要将Voxels容器Y方向的半长
    G4double ParaContainerHalfSizeZ = fVoxelLength*fHilbertCurve3DRepeatZ/2; // 这里是要将Voxels容器Z方向的半长
	G4cout<<"*********************************************************************************"<<G4endl;
	G4cout<<"The Hilbert curve data file name: " << HilbertCurveFileName <<G4endl;
	G4cout<<"The nucleus was devided into " << fHilbertCurve3DRepeatX << " subdomains(voxels) on X diection respectively."<<G4endl;
    G4cout<<"The nucleus was devided into " << fHilbertCurve3DRepeatY << " subdomains(voxels) on Y diection respectively."<<G4endl;
    G4cout<<"The nucleus was devided into " << fHilbertCurve3DRepeatZ << " subdomains(voxels) on Z diection respectively."<<G4endl;
	G4cout<<"Voxel size :"<< fVoxelLength/um << " um " <<G4endl;
	G4cout<<"Voxel container X size :" << ParaContainerHalfSizeX*2/um<<" um "<<G4endl;
    G4cout<<"Voxel container Y size :" << ParaContainerHalfSizeY*2/um<<" um "<<G4endl;
    G4cout<<"Voxel container Z size :" << ParaContainerHalfSizeZ*2/um<<" um "<<G4endl;
    ...
    param = new TsVoxelParameterisation(fPm); // 初始化体素参数控制器
    param->SetVoxelDimensions( fVoxelLength/2, fVoxelLength/2, fVoxelLength/2 ); // 初始化每个体素的半边长
    param->SetNoVoxel( fHilbertCurve3DRepeatX, fHilbertCurve3DRepeatY, fHilbertCurve3DRepeatZ ); // 设置x y z方向上体素的个数
    param->SetContainerDimensions(ParaContainerHalfSizeX, ParaContainerHalfSizeY, ParaContainerHalfSizeZ); // 设置整个容器的半边长
...
//	fFiberLogic->SetVisAttributes(FiberEnvelopeVis);
    ...
    G4int maxSize = fHilbertCurveLayer*TotalPoints*(G4int)(fHilbertCurve3DRepeatX*fHilbertCurve3DRepeatY*fHilbertCurve3DRepeatZ);
...
{
	G4double nVoxelX=fHilbertCurve3DRepeatX; 
	G4double nVoxelY=fHilbertCurve3DRepeatY;
	G4double nVoxelZ=fHilbertCurve3DRepeatZ; 
