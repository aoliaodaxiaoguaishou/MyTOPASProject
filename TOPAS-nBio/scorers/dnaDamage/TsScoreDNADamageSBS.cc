							...
							: TsVNtupleScorer(pM, mM, gM, scM, eM, scorerName, quantity, outFileName, isSubScorer)
{
	...
	fScoringRadiusX = 0;
    if (fPm->ParameterExists(GetFullParmName("ScoringRadiusX")))
    fScoringRadiusX = fPm->GetDoubleParameter(GetFullParmName("ScoringRadiusX"),"Length");

    fScoringRadiusY = 0;
    if (fPm->ParameterExists(GetFullParmName("ScoringRadiusY")))
    fScoringRadiusY = fPm->GetDoubleParameter(GetFullParmName("ScoringRadiusY"),"Length");

    fScoringRadiusZ = 0;
    if (fPm->ParameterExists(GetFullParmName("ScoringRadiusZ")))
    fScoringRadiusZ = fPm->GetDoubleParameter(GetFullParmName("ScoringRadiusZ"),"Length");
...
G4bool TsScoreDNADamageSBS::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
    ...
    if (fScoringRadiusX > 0 && fScoringRadiusY > 0 && fScoringRadiusZ > 0)
    {
    ...
    G4bool withinScoringRadius = (pow(pos.x()-fNucleusTransX, 2) / pow(fScoringRadiusX, 2)
                                +pow(pos.y()-fNucleusTransY, 2) / pow(fScoringRadiusY, 2)
                                +pow(pos.z()-fNucleusTransZ, 2) / pow(fScoringRadiusZ, 2)) < 1;
